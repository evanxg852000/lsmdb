...
impl WriteAheadLogger {
    pub(crate) fn open(dir: PathBuf, id: u64) -> LiteDbResult<Self> {
        let log_file_path = dir.join(format!("{:01$}-wal.log", id, 20));
    ...
    pub fn flush(&self) -> LiteDbResult<()> {
        self.file.write().flush().map_err(LiteDbError::from)
    }
    ...
    pub fn file_path(&self) -> PathBuf {
        self.dir.join(format!("{:01$}-wal.log", self.id, 20))
    }
}
